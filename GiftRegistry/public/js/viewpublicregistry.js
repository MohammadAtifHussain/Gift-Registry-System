...
//$(this).click("disabled",true);
  ...
  var settings = {
    "async": true,
    "crossDomain": true,
    "url": "https://localhost:5051/findPublicRegistry",
    "method": "POST",
    "headers": {
      "cache-control": "no-cache",
      "postman-token": "ac4c0af7-c823-b83f-4b44-8b751393d7c7"
  ...
  $.ajax(settings).done(function (response) {
    ...
    Object.keys(response).forEach(function(key,index) {
      newresponse.push(response[index].username);
    });
    newresponse = Array.from(new Set(newresponse))
    //alert(newresponse);
    // Object.keys(response).forEach(function(key,index) {
    //   populateTable(response[index].username);
    // });
    for (var i = 0; i < response.length; i++) {
      populateTable(newresponse[i]);
       }
